...
public class MedRequestManager implements  RequestController{
    ...
    public void start(MedRequest request) throws SQLException{
        request.start();
        mrd.changeMedRequest(request.getRequestID(), request.getMedicine(),request.getNodeID(),request.getEmployeeName(), request.getEmergency(), request.getStatusInt());
    }
