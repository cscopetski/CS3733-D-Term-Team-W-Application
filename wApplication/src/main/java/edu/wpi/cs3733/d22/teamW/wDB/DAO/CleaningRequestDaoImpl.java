...
public class CleaningRequestDaoImpl implements CleaningRequestDao {
    ...
    public ArrayList<Integer> CleaningRequestAtLocation(String nodeID) throws SQLException {
        ...
        ResultSet results = statement.executeQuery(String.format("SELECT REQID FROM CLEANINGREQUESTS WHERE NODEID = '%s'", nodeID));
    ...
    @Override
    public void changeCleaningRequest(Integer requestID, String itemID, String nodeID, Integer employeeID, Integer emergency, RequestStatus status)
            throws SQLException {
        statement.executeUpdate(
                String.format(
                        "UPDATE CLEANINGREQUESTS SET ITEMID='%s', NODEID='%s', EMPLOYEEID=%d, ISEMERGENCY=%d, REQSTATUS=%d, UPDATEDTIMESTAMP='%s' WHERE REQID=%d",
                        itemID, nodeID, employeeID, emergency, status.getValue(), new Timestamp(System.currentTimeMillis()), requestID));
    }
