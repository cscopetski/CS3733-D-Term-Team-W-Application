...
public class Game {
  ...
  private final Label highLabel;
    ...
    private void ateGhost() {
        if (pacmanX == redGhost.getX()
                && (pacmanY + speed == redGhost.getY() || pacmanY - speed == redGhost.getY())) {
            setGhost(redGhost, 120, 170, Color.RED, 5);
        } else if ((pacmanX - speed == redGhost.getX() || pacmanX + speed == redGhost.getX())
                && pacmanY == redGhost.getY()) {
            setGhost(redGhost, 120, 170, Color.RED, 5);
        } else if (pacmanX == redGhost.getX() && pacmanY == redGhost.getY()) {
            setGhost(redGhost, 120, 170, Color.RED, 5);
        }

        if (pacmanX == pinkGhost.getX()
                && (pacmanY + speed == pinkGhost.getY() || pacmanY - speed == pinkGhost.getY())) {
            setGhost(pinkGhost, 120, 150, Color.PINK, 10);

        } else if ((pacmanX - speed == pinkGhost.getX() || pacmanX + speed == pinkGhost.getX())
                && pacmanY == pinkGhost.getY()) {
            setGhost(pinkGhost, 120, 150, Color.PINK, 10);
        } else if (pacmanX == pinkGhost.getX() && pacmanY == pinkGhost.getY()) {
            setGhost(pinkGhost, 120, 150, Color.PINK, 10);
        }

        if (pacmanX == orangeGhost.getX()
                && (pacmanY + speed == orangeGhost.getY() || pacmanY - speed == orangeGhost.getY())) {
            setGhost(pinkGhost, 120, 110, Color.ORANGE, 20);
        } else if ((pacmanX - speed == orangeGhost.getX() || pacmanX + speed == orangeGhost.getX())
                && pacmanY == orangeGhost.getY()) {
            setGhost(pinkGhost, 120, 110, Color.ORANGE, 20);
        } else if (pacmanX == orangeGhost.getX() && pacmanY == orangeGhost.getY()) {
            setGhost(pinkGhost, 120, 110, Color.ORANGE, 20);
        }

        if (pacmanX == cyanGhost.getX()
                && (pacmanY + speed == cyanGhost.getY() || pacmanY - speed == cyanGhost.getY())) {
            setGhost(pinkGhost, 80, 130, Color.CYAN, 25);
        } else if ((pacmanX - speed == cyanGhost.getX() || pacmanX + speed == cyanGhost.getX())
                && pacmanY == cyanGhost.getY()) {
            setGhost(pinkGhost, 80, 130, Color.CYAN, 25);
