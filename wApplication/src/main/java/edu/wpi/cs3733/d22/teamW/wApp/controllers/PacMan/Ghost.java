...
public class Ghost {
  ...
  public Ghost(
      ...
      double y,
      double width,
      double height) // receive the layout, x, y coordinates and the width and height of the
  ...
  public Boolean caughtPacman(
      Circle pacman, int speed) // will check if the ghost has caught pacman depending on the speed
      {
    if ((this.centerX - speed == pacman.getCenterX() && this.centerY == pacman.getCenterY())
        || (this.centerX + speed == pacman.getCenterX() && this.centerY == pacman.getCenterY()))
      return true;
